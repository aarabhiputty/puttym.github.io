...
+ **Copy and Paste**
  1. `y` - Copy text within the given file (yank)
  2. `p` - Paste the contents from local buffer (yanked text)
  3. `"` `+` `p` - Paste the contents from the clip board 
  4. `"` `1` `p` - Paste the contents of the Register "1 ("0 will always have the content of the latest yank, and the others will have last 9 deleted text, being "1 the newest, and "9 the oldest)  
